				...
				for(ShopOrderdetail orderdetail:orderdetails){
					...
					if(mydis != null){//我不是会员
						if(distributora.getExStatus()!=1 && distributora.getStatus()==1){//我是微店
							...
							disCommissiona += (commissionConfigService.getCommissionConfigByProductId(orderdetail.getPro(), 1))*num;//一级佣金
							
							if(distributorb!=null){
								if(distributorb.getExStatus() != 1){
									disCommissionb+= (commissionConfigService.getCommissionConfigByProductId(orderdetail.getPro(), 2))*num;//二级佣金
								}
							}
						...
						if(distributora.getExStatus()==1){//直属于加盟店
							...
							disCommissiona += (commissionConfigService.getCommissionConfigByProductId(orderdetail.getPro(), 1))*num;//
							if(topDistributor != null){
								if(spec!=null){
									topDisCommission += (s_Store_price.subtract(s_TydAmt).doubleValue())*num; 
								}else{
						...
						if(distributorb != null){
							if(distributorb.getExStatus() != 1){
								disCommissionb+= (commissionConfigService.getCommissionConfigByProductId(orderdetail.getPro(), 2))*num;
							}
						}
					}
				}
		...
		if(f!=null){
			...
			if(ss != null){
				...
				if(dis != null){
					...
					WeixinBaseUtils.sendMsgToFollower(a, f,"【舟山海鲜之家控股有限公司】尊敬的客户"+member.getNickname()+"您好！您于"+order.getCeateDate()+"所下订单:"+order.getCode()+"已成功了，"+address+tel+"，如需服务，请致电400-9261088");
				}else{
					WeixinBaseUtils.sendMsgToFollower(a, f,"【舟山海鲜之家控股有限公司】尊敬的客户"+member.getNickname()+"您好！您于"+order.getCeateDate()+"所下订单:"+order.getCode()+"已成功，如需服务，请致电400-9261088");
				}
			}else{
