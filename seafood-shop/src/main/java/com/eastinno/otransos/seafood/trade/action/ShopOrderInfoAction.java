    ...
    public Page doSendPro(WebForm form){
    	...
    	if(expressCode !=null && !expressCode.equals("") && expressid !=null && !expressid.equals("")){
    		...
    		if(f!=null){
    			...
    			WeixinBaseUtils.sendMsgToFollower(a, f, "【舟山海鲜之家控股有限公司】尊敬的客户"+member.getNickname()+"您好！您的订单:"+order.getCode()+"已经发出，"+order.getLogisticsCompany().getName()+"快递单号："+order.getExpressCode()+"，请保持电话通畅，您可在商城的订单内查询实时物流信息，收货时请当面验货。也可随时致电400-9261088查询物流信息。");
    		}
    	}
    	return go("sendList");
    }
