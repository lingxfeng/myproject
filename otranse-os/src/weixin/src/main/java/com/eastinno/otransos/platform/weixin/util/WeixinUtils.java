...
public class WeixinUtils {
	...
	public static void setMsgInfo(RespMessage oms, ReqMessage msg)
			throws Exception {
		// 设置发送信息
	 ...
	 * @throws Exception
	 */
	public static ReqMessage parseXmlToReqM(HttpServletRequest request)
			throws Exception {
		// 将解析结果存储在ReqMessage中
	 ...
	 */
	public static String getViewButionUrl(String url, String appId,
			String menuKey) {
		url = URLEncoder.encode(url);
		return "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + appId + "&redirect_uri=" + url
				+ "&response_type=code&scope=snsapi_userinfo&state=" + menuKey
				+ "#wechat_redirect";
	}
	...
	public static String jsnonceStr(Map<String, String> map) {
		...
		String contentstr = content.toString().substring(0,
	 ...
	 */
	 public static JSONObject httpRequest(String reqUrl, String reqMethod, String ops) {
	     ...
	     * @throws UnsupportedEncodingException
	     */
	    public static Map<String, Object> menuToMap(List<Menu> menus, Map<String, Object> map, Account account, boolean isParent) {
	        ...
	        for (Menu m : menus) {
	            ...
	            if (chilren != null && chilren.size() > 0) {
	                ...
	                if ("viewin".equals(m.getType())) {
	                    ...
	                    String url = WeixinUtils.getViewButionUrl(domain + "/" + m.getUrl() + "&accountId=" + account.getId(), account.getAppid(), m.getMenuKey());
	                    mapmenu.put("url", url);
